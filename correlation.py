...
def compute_correlation(list_ord,window,phase,Kp,Vsys):
    ...
    for kk in range(nord_tmp):                           # For each order:
        ...
        fit_px = np.polyfit(list_ord[kk].W_red,std_px,2) # Fit polynomial of order 2 to the distribution (blaze function)
        Stdfit = np.poly1d(fit_px)                       
        Stdtot.append(Stdfit(list_ord[kk].W_red))        # Store best-fitting solution
        wl.append(list_ord[kk].W_red)                    # Store wavelength solution
    ...
    for no in range(nord_tmp):   # For each order
        for ii in range(Nkp):    # For each value of Kp
            for jj in range(Nv): # For each value of Vsys
                vp               = src.rvp(phase,Kp[ii],Vsys[jj])   # Compute planet RV
                modelij          = np.zeros((len(wl[no]),len(phase)))                         
                interpmod        = F2D[no](vp) # Use the pre-computed grid to compute the sequence of spectra
